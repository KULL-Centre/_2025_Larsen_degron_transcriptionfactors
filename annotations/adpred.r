...
tf = read.csv(gzfile("transfac_tiles.csv.gz"))
...
adp_raw = read.table(gzfile("adpred_ss_api_full_scores.txt.gz"))
...
# when using this on CT tiles of length 29, it means that these are in practice added a CT Gly. We bravely move no
...
# Dump adpred annotation
tf$tad = F
tf[icen,"tad"] = T
write.csv(tf[,c("name","aa","tad","adp_cen")], file="transfac_tiles_tad.csv", quote=F, row.names=F)

# Distribution of all per-residue scores
h_adp_api = hist(unlist(adp), breaks=100, plot=F)

# # Read ADPred predictions of full proteins assuming all secondary structure is coil
# adp_coil_raw = read.table("adpred/adpred_ss_coil_full_scores.txt")
# i = which(adp_coil_raw$V1=="AC092835.1")
# adp_coil_raw[i,"V1"] = "ZN892"
# print(sprintf("Changed %d gene name from AC092835.1 to ZN892",length(i)))
# i = which(adp_coil_raw$V1=="T")
# adp_coil_raw[i,"V1"] = "TBXT"
# print(sprintf("Changed %d gene name from T to TBXT",length(i)))

# adp_coil = strsplit(adp_coil_raw$V3, ";")
# adp_coil = lapply(adp_coil, as.numeric)
# names(adp_coil) = adp_coil_raw$V1
# h_adp_coil = hist(unlist(adp_coil), breaks=100, plot=F)
...
# Paper plot
pdf("adpred_high_scoring.png", width=3.5, height=3.5, pointsize=7)
n=5
breaks=60
cn = "degron_score"
hs = hist(tf[,cn],     breaks=breaks, plot=F)
...
breaks = 50
# hs  = hist(tf[,cn],     breaks=breaks, plot=F)
# hha = hist(tf[iavg,cn], breaks=breaks, plot=F)
# hhm = hist(tf[imax,cn], breaks=breaks, plot=F)
# hhc = hist(tf[icen,cn], breaks=breaks, plot=F)
bw = 2e-2
...
# # plot(0,0,col=0, xlim=c(-.3,1.4), ylim=c(0,2.7), xlab="Degron score", ylab="Density")
# plot(0,0,col=0, xlim=hs$mids[c(1,length(hs$mids))], ylim=c(0,max(hha$density,hs$density)), xlab=cn, ylab="Density")
# lines(hha$mids, hha$density, col=2)
# lines(hs$mids, hs$density, col=1)
...
# # plot(0,0,col=0, xlim=c(-.3,1.4), ylim=c(0,2.7), xlab="Degron score", ylab="Density")
# plot(0,0,col=0, xlim=hs$mids[c(1,length(hs$mids))], ylim=c(0,max(hhm$density,hs$density)), xlab=cn, ylab="Density")
# lines(hhm$mids, hhm$density, col=2)
# lines(hs$mids, hs$density, col=1)
...
# # plot(0,0,col=0, xlim=c(-.3,1.4), ylim=c(0,2.7), xlab="Degron score", ylab="Density")
# plot(0,0,col=0, xlim=hs$mids[c(1,length(hs$mids))], ylim=c(0,max(hhc$density,hs$density)), xlab=cn, ylab="Density")
# lines(hhc$mids, hhc$density, col=2)
# lines(hs$mids, hs$density, col=1)
...
# plot(0,0,col=0, xlim=hs$x[c(1,length(hs$x))], ylim=c(0,max(hpm$y,hp$y,hs$y)), xlab=cn, ylab="Density")
...
# plot(0,0,col=0, xlim=hs$x[c(1,length(hs$x))], ylim=c(0,max(hpm$density,hp$density,hs$density)), xlab="PAP score", ylab="Density")
